...
module.exports.run = async (bot,message,args,prefix,con) => {
    ...
    if (!loveUser) return React.sendReact(false,message,"You did not specify a user mention or ID!","reply");
    if (loveUser.id === bot.user.id) return React.sendReact(true,message,"100000000000% :heart: :heart: :heart: :kissing_heart: ","reply");
    ...
    React.sendReact(true,message,`${lovingUser} and ${loveUser} love each other at ${lovePercent}% :heart:`,"send");
    return;
}
