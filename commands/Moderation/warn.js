...
module.exports.run = async (bot, message, args, prefix, db, logChannel) => {
  ...
  let warningUser = message.guild.member(
  ...
  if (!warningUser.kickable)
    return React.sendReact(
      false,
      message,
      "That person can't be warned! (Do they have a higher role? Do I have require permissions?)",
      'reply'
    );
  if (!reason)
    return React.sendReact(false, message, 'You must give a reason!', 'reply');

  if (!bot.warns[warningUser.id])
    bot.warns[warningUser.id] = {
      warnings: 0
    };
  ...
  React.sendReact(
    true,
    message,
    `${warnUser.user.username} has been banned warned *${reason}*`,
    'send'
  );
  ...
  if (bot.warns[warningUser.id].warnings === 3) {
    ...
    await warningUser.addRole(role).catch(error => {
      return React.sendReact(
        false,
        message,
        `Couldn't mute user because of: ${error}(He has got 3 warns)`,
        'reply'
      );
    });
  ...
  if (bot.warns[warningUser.id].warnings === 5) {
    await message.guild
      .member(warningUser)
      .ban('User have got 5 warnings from administrators')
      .catch(error => {
        return React.sendReact(
          false,
          message,
          `Couldn't ban user because of: ${error}(He has got 5 warns)`,
          'reply'
        );
      });
    if (logChannel) {
