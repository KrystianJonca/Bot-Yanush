...
module.exports.run = async (bot,message,args,prefix,con,logChannel) => {
    ...
    if (!muteUser.kickable) 
        return React.sendReact(false,message,"That person can't be muted! (Do they have a higher role? Do I have require permissions?)","reply");
    if (!reason) 
    ...
    if (muteUser.roles.has(role.id)) 
        return React.sendReact(false,message,"This user is already muted!","reply");
    ...
    await muteUser.addRole(role).catch(error => {
        return React.sendReact(false,message,`Couldn't mute user because of: ${error}`,'reply');
    });
    if(logChannel){
    ...
    React.sendReact(true,message,`${muteUser.user.username} has been muted because *${reason}*`,'reply');
}
