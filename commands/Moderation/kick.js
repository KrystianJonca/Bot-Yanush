...
module.exports.run = async (bot, message, args, prefix, db, logChannel) => {
  ...
  if (!kickUser.kickable)
    return React.sendReact(
      false,
      message,
      "That person can't be kicked! (Do they have a higher role? Do I have require permissions?)",
      'reply'
    );
  if (!reason)
  ...
  await message.guild
    ...
    .catch(error => {
      return React.sendReact(
        false,
        message,
        `Couldn't kick user because of: ${error}`,
        'reply'
      );
    });
  if (logChannel) {
  ...
  React.sendReact(
    true,
    message,
    `${kickUser.user.username} has been banned because *${reason}*`,
    'send'
  );
};
