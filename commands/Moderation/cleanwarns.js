...
module.exports.run = async (bot,message,args,prefix,con,logChannel) => {
    ...
    if (!cleanUser.kickable) 
        return React.sendReact(false,message,"That person can't be cleared! (Do they have a higher role? Do I have require permissions?)","reply");
    if (!reason)   
    ...
    React.sendReact(true,message,`Warns cleared for ${cleanUser.user.username}`,"send");
}
