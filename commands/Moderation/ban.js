...
module.exports.run = async (bot, message, args, prefix, Guild, logChannel) => {
  ...
  if (!banUser.bannable)
    return React.sendReact(
      false,
      message,
      "That person can't be baned! (Do they have a higher role? Do I have require permissions?)",
      'reply'
    );
  if (!reason)
  ...
  await message.guild
    ...
    .catch(error => {
      return React.sendReact(
        false,
        message,
        `Couldn't ban user because: ${error}`,
        'reply'
      );
    });
  if (logChannel) {
  ...
  React.sendReact(
    true,
    message,
    `${banUser.user.username} has been banned because *${reason}*`,
    'send'
  );
};
