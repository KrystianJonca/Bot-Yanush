...
module.exports.run = async (bot,message,args) => {
    ...
    if (!messageToDelete || isNaN(messagesToDelete)) 
        return React.sendReact(false,message,"You must enter the number of messages to delete!","reply");
    if (messagesToDelete > 100) 
        return React.sendReact(false,message,`Number of messages to delete must be less or equal to ${messageToDeleteLimit}!`,"reply");
    try{
