...
module.exports = async (bot,con) => {
    ...
    bot.setInterval(async () => {
        for (let i in bot.mutes) {
            ...
            let logChannelId = await Database.getLogChannel(message.guild,con);
    ...
    bot.setInterval(async () => {
        con.query('SELECT * FROM twitch',async (err,rows) => {
            ...
            for(i in rows){
                ...
                const streamRes = await fetch(`https://api.twitch.tv/kraken/streams/${username}`,{
                    method: 'GET',
                    headers: {
                        'Client-ID': CLIENT_ID,
                    }
                ...
                let msg = message.replace('{{ streamer }}',stream.stream.channel.display_name).replace('{{ link }}',`https://www.twitch.tv/${username}`)
                
                let embed = new Discord.RichEmbed()
                    .setAuthor(stream.stream.channel.display_name,stream.stream.channel.logo)
