...
bot.on('message',async message =>{
    ...
    if (coolDown.has(sender.id)) {
        //message.react(otherSettings.reaction_of_the_command_not_execution);
...
function commandInfo(cmd,message,prefix){
    fs.readdir('./commands/',(err,files) =>{
        ...
        jsfiles.forEach((f,i) => {
            let cmds = require(`./commands/${f}`);
            if (cmds.config.enabled) {
                if (cmds.config.name.indexOf(cmd) != -1) {
                    commandExist = true;
        ...
        if (!commandExist) {
            message.reply(`This command does not exist or starts with incorrect prefix! Type \`${prefix}help\` to get the avaiable command list!`);
            return;   
        }
    });
    return;
